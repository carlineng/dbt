...
from dbt.contracts.graph.unparsed import (
    UnparsedSourceDefinition, UnparsedNodeUpdate, UnparsedMacroUpdate,
    UnparsedAnalysisUpdate, HasTests, UnparsedSourceTableDefinition,
    FreshnessThreshold
)
...
class SchemaParser(SimpleParser[SchemaTestBlock, ParsedTestNode]):
    ...
    def parse_column_tests(self, block: TestBlock, column: HasTests) -> None:
        if not column.tests:
            return

        for test in column.tests:
            self.parse_test(block, test, column.name)
